...
async def _dispatch(name: str, args: dict) -> dict:
    ...
    async with async_session() as session:
        ...
        if name in ("pavo_render_video","pavo_export_project","pavo_get_video_status"):
            import uuid
            try: pid = uuid.UUID(args.get("project_id",""))
            except: return MCPToolResult.fail("INVALID_PARAM","Invalid project_id").model_dump()
            stmt = select(Project).where(Project.id == pid)
            r = await session.execute(stmt); proj = r.scalar_one_or_none()
            if not proj: return MCPToolResult.fail("NOT_FOUND","Project not found").model_dump()
            if name == "pavo_render_video":
                from app.services.project_service import ProjectService
                svc = ProjectService(session); vr = await svc.render_video(proj)
                ...
                content = storyboard_to_markdown(proj) if fmt == "markdown" else storyboard_to_script(proj) if fmt == "script" else None
                return MCPToolResult.ok({"format":fmt,"content":content,"filename":f"storyboard.{fmt}"}).model_dump() if content else MCPToolResult.fail("INVALID_PARAM","Unsupported format").model_dump()
            else:
...
async def read_resource(uri: str):
    ...
    async with async_session() as session:
        if uri == "pavo://projects":
            ...
            data = [{"id":str(p.id),"title":p.title,"status":p.status.value if hasattr(p.status,'value') else p.status,"created_at":p.created_at.isoformat() if p.created_at else None} for p in r.scalars().all()]
            return json.dumps(data, ensure_ascii=False)
        if uri.startswith("pavo://projects/"):
            parts = uri.split("/"); pid = parts[2]
            try:
                r = await session.execute(select(Project).where(Project.id == uuid.UUID(pid)))
                p = r.scalar_one_or_none()
                if not p: return json.dumps({"error":"not found"})
            except: return json.dumps({"error":"invalid id"})
            d = {"id":str(p.id),"title":p.title,"status":p.status.value if hasattr(p.status,'value') else p.status,"input":p.input_raw}
            if len(parts)>3:
                sub = parts[3]
                if sub=="storyboard": return json.dumps(p.storyboard or {}, ensure_ascii=False)
                if sub=="characters": return json.dumps(p.characters or [], ensure_ascii=False)
            return json.dumps(d, ensure_ascii=False)
        if uri.startswith("pavo://users/"):
            parts = uri.split("/"); uid = parts[2]
            from .tools.memory_tools import MemoryTools
            mt = MemoryTools(session)
            ...
            return json.dumps({"memories":mems} if len(parts)<=4 else (mems[0] if mems else {}), ensure_ascii=False)
