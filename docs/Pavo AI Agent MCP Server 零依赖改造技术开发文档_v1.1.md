...
**文档版本：** v1.1  
**日期：** 2026年7月13日  
**修订说明：** 根据代码审查意见修订，补充 `set_config`、`task_done` 修正、VectorStore 抽象接口、完整建表 SQL、静态服务启动代码、全局兜底机制及迁移说明。
...
class AsyncTaskQueue:
    ...
    async def submit(self, coro: Callable, *args, **kwargs) -> str:
        ...
        while True:
            ...
            try:
                async with self.semaphore:
                    ...
                    await self._update_status(task.id, "done", result=result)
            except Exception as e:
                await self._update_status(task.id, "failed", error=str(e))
            finally:
    ...
    async def _update_status(self, task_id: str, status: str, result=None, error: str = None):
        ...
        async with async_session() as session:
            await session.execute(
                text("""
                    ...
                    VALUES (:task_id, 'video_render', :status, :result_path, :error, CURRENT_TIMESTAMP)
...
class SqliteVecStore(VectorStore):
    ...
    async def upsert(self, id: str, vector: List[float], metadata: dict) -> None:
        async with async_session() as session:
            await session.execute(
                text("""
                    INSERT INTO vec_memories(id, embedding, metadata)
                    VALUES (:id, :vec, :meta)
                    ON CONFLICT(id) DO UPDATE SET
                        embedding = :vec, metadata = :meta
                """),
                {"id": id, "vec": json.dumps(vector), "meta": json.dumps(metadata)}
            )
            await session.commit()
...
| **Phase 4** 配置管理与封装 | 1. 实现 `get_config` / `set_config`<br>2. 实现 `VectorStore` 抽象接口<br>3. 创建 CLI 入口（含权限检查、静态服务启动） | `app/config.py`<br>`app/vectorstore/`<br>`pavo_mcp_server/cli.py` | 统一配置管理 + 一键启动命令 |
