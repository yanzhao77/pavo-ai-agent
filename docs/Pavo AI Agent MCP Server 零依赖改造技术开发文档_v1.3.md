...
**文档版本：** v1.3  
**日期：** 2026年7月13日  
**修订说明：** 在 v1.2 基础上修正日志重复输出问题、`pavo-start` 缺少 Node.js 环境检查问题；新增 `vec_memories_vec` 主键约束说明、`filters` 参数扩展说明及双表事务一致性确认说明。
...
-- 注意 1：维度 1536 需与实际 Embedding 模型输出维度保持一致
-- 注意 2：vec0 虚拟表默认将 id 字段作为主键，INSERT ... ON CONFLICT(id) DO UPDATE
--          语法依赖此主键约束，无需额外创建索引。
...
class SqliteVecStore(VectorStore):
    ...
    事务一致性说明：
        upsert 中两次 execute 均在同一个 async_session 上下文内，最后的
        commit() 会原子提交两次操作，不存在半写问题。

    filters 扩展说明：
        search 接口已预留 filters 参数，当前实现未使用。
        后续迭代可在 search 中对 JOIN 后的 metadata 字段添加
        WHERE 子句实现内容过滤（如按项目 ID、记忆类型过滤）。
    """
...
def setup_logging(pavo_home: Path):
    ...
    注意：不使用 logging.basicConfig() 直接指定 filename，
    避免其默认行为导致 StreamHandler 被重复添加。
    """
    log_dir = pavo_home / "logs"
    log_dir.mkdir(exist_ok=True)

    # 配置文件 Handler：详细格式，包含时间戳和模块名
    file_handler = logging.FileHandler(
        log_dir / "pavo.log", encoding="utf-8"
    )
    ...
    # 配置根 logger：明确添加两个 Handler，避免重复输出
...
def start(mode: str, frontend_port: int):
    ...
    前置条件：需要 Node.js 18+ 并确保 npm 在 PATH 中。
    如果仅使用 MCP 功能，请改用 pavo-mcp-server 命令。
    """
    task_queue = _boot(mode)

    # 前置检查：确认 npm 命令可用，缺失时给出友好提示
    try:
        ...
        logging.error(
            "未找到 npm 命令。请安装 Node.js 18+，并确保 npm 在 PATH 中。\n"
            "如果仅需使用 MCP 功能，请改用 pavo-mcp-server 命令。"
        )
        sys.exit(1)
